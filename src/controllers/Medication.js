...
export const deleteMedication = async (req, res) => {
    ...
    try {
        ...
        // Cek jika monitoring period sudah expired
        if (monitoringPeriod.status === 'expired') {
            return res.status(400).json({ msg: "Cannot stop notifications. Monitoring period is expired." });
        }
        // Cari medication berdasarkan ID dan monitoringPeriodId
        const medication = await Medications.findOne({
            where: { id, monitoringPeriodId },
        ...
        // Menonaktifkan semua notifikasi terkait obat
        const updatedNotifications = await Notifications.update(
            { status: 'inactive' }, 
            { where: { medicationId: medication.id } } 
        );
        // Periksa jika tidak ada notifikasi yang diubah
        if (updatedNotifications[0] === 0) {
            return res.status(404).json({ msg: "No active notifications found for this medication" });
        }
        res.status(200).json({ msg: "Notifications for the medication have been stopped successfully" });
    } catch (error) {
        console.error("Error stopping notifications:", error);
        res.status(500).json({ msg: "An error occurred while stopping notifications for the medication" });
    }
};
